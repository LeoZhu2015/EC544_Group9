...
int irPin = 2;
int irPin1 = 3;
boolean autoMode_OR_ControlMode = false;
double irValue1;
...
double aggKp=1, aggKi=0, aggKd=0.2;
double consKp=0.9, consKi=0, consKd=0.1;
String incomingByte="w";
...
void processResponse(String command) {
       if (command == "x" ) { //autoMode_OR_ControlMode
...
//        Serial.println(getDistance(irPin));
        if(distanceB>250){
          if(irValue1>150){
            esc.write(90);
            delay(1000);
            double distanceA = get_front_distance();
            if(distanceA<250||distanceA>700){
            ...
            else if(distanceA>=250&&distanceA<700){
...
//    Serial.print("\n");
    ...
    if(dist>100){
      myPID_Distance.SetTunings(aggKp,aggKi,aggKd);
    }
    else{
